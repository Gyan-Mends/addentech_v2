...
const MonthlyReportsList = () => {
  ...
  const canViewReport = (report: Report) => {
    ...
    // Admin and manager can see ALL reports (including drafts)
    if (userRole === 'admin' || userRole === 'manager') {
      console.log(`${userRole} check: true (status: ${reportStatus}) - can see all reports`);
      return true;
    }
  ...
  const determineDepartmentType = (deptName: string) => {
    ...
    // Check for data department first (most specific)
    if (name.includes('data') && !name.includes('customer')) {
      return 'data';
    }
    
    // Check for customer service (must contain both 'customer' and 'service' or specific variations)
    if (name.includes('customer service') || 
        name.includes('customer support') || 
        name.includes('client service') ||
        (name.includes('customer') && name.includes('service'))) {
      return 'customer_service';
    }
    
    // Check for software/development departments
    ...
    // Check for news/media departments
    ...
    // Default to data for unmatched departments (since data is most common)
    return 'data';
  };
